...
### 문제점
- 각각의 트랜잭션과 락을 서비스 별로 분리하였기 때문에, 만일 주문 과정 중에서 도중에 예외가 발생한다면, 호출부에서 전체 트랜잭션을 관장하지 못하기 때문에 이를 관리해줄 방법이 필요합니다.
...
- 추후 MSA 구조로 확장될 경우에는 보다 서비스 간의 결합도를 낮출 수 있도록 `이벤트` 기반의 `비동기적` 방식이나, 
- 적절한 혼합방식을 통해 주요 로직은 `동기적` 방식 부가 로직은 `비동기적` 방식 등을 이용하여 확장에 용이한 코드를 설계하는 것이 목표 입니다!
